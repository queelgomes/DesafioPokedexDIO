...
pokeApi.getPokemons = (offset = 0, limit = 5) => {
    ...
    // O retorno do fetch sempre sera o response. Abaixo transformamos ele em json.
    return fetch(url)
        .then((response) =>response.json())
        // Abaixo usamos o results la do código para ter a lista de pokemons
        .then((jsonBody) => jsonBody.results)
        //Abaixo criamos uma nova lista com todos os pokemons do limit. 
        //Como esta em outro url, criamos o pokeApi.getPokemon.
        .then((pokemons) => pokemons.map(pokeApi.getPokemon)) 
        // Acima ele transformou em uma lista cada pokemon e para não criar um then de cada pokemon (cada indice da lista), fazemos o Promise.
        // Ele carrega todo o código. Por isso é .all
        .then((detailRequests) => Promise.all(detailRequests))
        // Abaixo criamos uma função para retornar todo Promise carregado.
